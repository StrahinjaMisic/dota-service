...
public class UseEventParserTest {
    ...
    public void testParseEvent() {
        ...
        useEventParser.parseEvent("[00:25:40.545] npc_dota_hero_death_prophet uses item_phase_boots", match);
        verify(heroService, times(1)).getOrCreate(heroName);
        verify(itemService, times(1)).getOrCreate(itemName);
        verify(timeUtil, times(1)).eventTimeToMilliseconds("00:25:40.545");
    }

    @Test
    public void testParseEventWhenEventIsIgnored() {
        Match match = DotaTestFactory.createMatch();
        useEventParser.parseEvent("[00:26:04.306] npc_dota_hero_snapfire buys item item_ward_observer", match);
